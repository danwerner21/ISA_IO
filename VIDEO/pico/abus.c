...
static void abus_main_setup(PIO pio, uint sm) {
    ...
    // set the bus WR pin as the jump pin
    sm_config_set_jmp_pin(&c, CONFIG_PIN_APPLEBUS_WR);
...
static void shadow_memory(bool is_write, uint_fast16_t address, uint32_t value) {
    ...
    if(address<=0x0D)
    {
        device_write(address, value & 0xff);
    }
...
void abus_loop() {
    while(1) {
        ...
        const bool is_write = ((value & (1u << (CONFIG_PIN_APPLEBUS_WR - CONFIG_PIN_APPLEBUS_DATA_BASE))) == 0);
